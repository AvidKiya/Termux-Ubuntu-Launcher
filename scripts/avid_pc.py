...
def main():
    ...
    if ns.cmd=="desktop": return subprocess.call([sys.executable, str(ROOT/"scripts"/"avid_desktop.py")])
    if ns.cmd=="doctor": return subprocess.call([sys.executable, str(ROOT/"tools"/"qa_selftest.py")])
    if ns.cmd=="build-exe":
        script = ROOT/"packaging"/"windows"/("build_exe.ps1" if os.name=="nt" else "build_exe.sh")
        if os.name=="nt": return subprocess.call(["powershell","-ExecutionPolicy","Bypass","-File",str(script)])
